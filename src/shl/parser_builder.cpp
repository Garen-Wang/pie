...
namespace shl {
  ...
  void Colors::append(const std::string& color, const QColor& _color) { colors[color] = _color; }
  bool Colors::exist(const std::string& str) { return colors.count(str) != 0; }
  QColor Colors::get(const std::string& str) { return colors[str]; }

  ParserBuilder *generateParserBuilder(Colors& colors, Rope *rope) {
    ParserBuilder *g = new ParserBuilder();
