...
namespace {
  ...
  public:
    ...
    explicit State(const std::string_view &s, size_t c = 0)
        : string(s), position(c), maxPosition(c), singleString(true) {}
    explicit State(StringViews s, size_t c = 0)
        : svs(std::move(s)), position(c), maxPosition(c), singleString(false) {}
    ...
    grammar::Letter current() const {
      if (singleString) {
        ...
        //        auto x = std::upper_bound(svs.prefixSums.begin(), svs.prefixSums.end(), position)
        //        - svs.prefixSums.begin(); std::cout << "x is " << x << std::endl;
  ...
  std::shared_ptr<SyntaxTree> parseRule(const std::shared_ptr<grammar::Rule> &rule, State &state,
    ...
    if (state.singleString)
      syntaxTree = std::make_shared<SyntaxTree>(rule, state.string, state.getPosition());
    else
      syntaxTree = std::make_shared<SyntaxTree>(rule, state.svs, state.getPosition());
...
std::size_t StringViews::getStringSize() const { return strings.size(); }
...
std::vector<std::string_view>::iterator StringViews::begin() { return strings.begin(); }
std::vector<std::string_view>::iterator StringViews::end() { return strings.end(); }
